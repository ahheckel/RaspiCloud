...
for ((i = 0; i < ${#syncfolders[@]}; i++)) ; do
    ...
    if [ "$(cat $md5)" != "$(cat $_md5)" ] ; then 
		...
		rsync $opts "$dir"/* --exclude='*.*.part' --exclude='*.*.crdownload' --exclude=".*" --exclude='~*' "${md5excl[@]}" --iconv=utf-8,ascii//TRANSLIT//IGNORE -e "ssh -i $ckey" ${user}@$ip:$dstdir/.${dev}${md5n}/ | grep ^\<f | cut -d " " -f 2- > $HOME/.dirlists/.update.list
